...
from .humanml.data.dataset import Text2MotionDatasetMotionX, Text2MotionDatasetMotionX_with_face, Text2MotionDatasetMotionX_text_all, VQMotionDataset_MotionX, Text2MotionDatasetMotionX_dual_codebook, Text2MotionDatasetMotionX_VQToken


class Motion_X_V26_DataModule(BASEDataModule):
    ...
    def __init__(self,
        ...
        assert eval(f'cfg.{phase.upper()}.DATASETS')[0] == 'motionx_v26'

        self.name = "motionx_v26"

        if cfg.DATASET.JOINT_TYPE == 'humanml3d':
            self.njoints = 22
            self.hparams['njoints']=22
        elif cfg.DATASET.JOINT_TYPE in ['motionx', 'motionx_v26']:
            ...
            if 'MINOR_MOTION_TYPE' in cfg.DATASET:
                ...
                if cfg.DATASET.MOTION_TYPE in ['vector_263', 'vector_263_ori_humanml']:
                    self.njoints = 22
                    self.hparams['njoints']=22
                else:
                    self.njoints = 52
        ...
        if phase == "text_only":
            ...
            if cfg.TRAIN.STAGE in ['gpt'] and (not cfg.TEST.inference_vq_code):
                ...
                if cfg.model.condition in ['text_all', 'text_face', 'text_body', 'text_hand', 'text_face_body', "text_seperate", "only_pose_concat", "only_pose_fusion"]:
                    ...
                    if cfg.model.vae_type in ['hvq_body_hand_face']:
                        self.Dataset = Text2MotionDatasetMotionX_with_face
                    else:
    ...
    def feats2joints(self, features, motion_type, skel=None, smplx_model=None):
        # import pdb; pdb.set_trace()
        if motion_type in ['all', 'root_body_pos_vel_hand_all', 'root_body_pos_vel_hand_pos_vel', 'root_body_pos_vel_hand_pos', 'vector_263_ori_humanml']:
