...
from .humanml.data.dataset import Text2MotionDatasetMotionX, Text2MotionDatasetMotionX_text_all, VQMotionDataset_MotionX, Text2MotionDatasetMotionX_dual_codebook, Text2MotionDatasetMotionX_VQToken, \
                                Text2MotionDatasetMotionX_body_hand_codebook, Text2MotionDatasetMotionX_with_face
...
class Motion_XDataModule(BASEDataModule):
    ...
    def __init__(self,
        ...
        assert eval(f'cfg.{phase.upper()}.DATASETS')[0] == 'motionx'
        ...
        if phase == "text_only":
            ...
            if cfg.TRAIN.STAGE in ['gpt'] and (not cfg.TEST.inference_vq_code):
                ...
                elif cfg.model.vae_type in ['dual_human_vq']:
                    self.Dataset = Text2MotionDatasetMotionX_body_hand_codebook
                else:
    ...
    def facerenorm2ori(self, features):
        # mean = torch.tensor(self.hparams.mean).to(features)
        # std = torch.tensor(self.hparams.std).to(features)

        self.face_mean = torch.tensor(self.face_mean).to(features)
        self.face_std = torch.tensor(self.face_std).to(features)
        features = features * (self.face_std + 1e-7)  + self.face_mean
        return features
