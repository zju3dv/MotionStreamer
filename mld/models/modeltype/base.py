                    ...
                    for i in range(len(outputs)):
                        if self.vae_type in ["hvq", "hvq_body_hand"]:
                            for bid in range(
                                ...
                                output_dir_t = Path(
                                    os.path.join(f'/comp_robot/lushunlin/datasets/{cfg.TEST.DATASETS[0]}/vq_tokens', str(cfg.model.vae_type), 'motion_vqcode_t'))
                                output_dir_b = Path(
                                    os.path.join(f'/comp_robot/lushunlin/datasets/{cfg.TEST.DATASETS[0]}/vq_tokens', str(cfg.model.vae_type), 'motion_vqcode_b'))
                            ...
                            for bid in range(
                                ...
                                motion_name = f"{motion_name}.npy"
                                output_dir = Path(
                                    os.path.join(f'/comp_robot/lushunlin/datasets/{cfg.TEST.DATASETS[0]}/vq_tokens', str(cfg.model.vae_type)))
                                # save predictions results
                ...
                if cfg.TEST.inference_vq_code:
                        ...
                        if self.vae_type in ["hvq", "hvq_body_hand"]:
                            for bid in range(
                                ...
                                if cfg.TEST.DATASETS[0].lower() == 'motionx_v26':
                                    output_dir_t = Path(
                                        os.path.join(f'/comp_robot/lushunlin/datasets/Motion-X-V26/vq_tokens', str(cfg.model.vae_type), 'motion_vqcode_t'))
                                    output_dir_b = Path(
                                        os.path.join(f'/comp_robot/lushunlin/datasets/Motion-X-V26/vq_tokens', str(cfg.model.vae_type), 'motion_vqcode_b'))
                                elif cfg.TEST.DATASETS[0].lower() == 'motionx':
                                    output_dir_t = Path(
                                        os.path.join(f'/comp_robot/lushunlin/datasets/Motion-X/vq_tokens', str(cfg.model.vae_type), 'motion_vqcode_t'))
                                    output_dir_b = Path(
                                        os.path.join(f'/comp_robot/lushunlin/datasets/Motion-X/vq_tokens', str(cfg.model.vae_type), 'motion_vqcode_b'))
                                else:
                                    raise NotImplementedError
                                # save predictions results
                            ...
                            for bid in range(
                                ...
                                motion_name = f"{motion_name}.npy"
                                output_dir = Path(
                                    os.path.join(f'/comp_robot/lushunlin/datasets/Motion-X/vq_tokens', str(cfg.model.vae_type)))
                                # save predictions results
                    ...
                    for i in range(len(gen_motions)):
                        for bid in range(
                            ...
                            # textpath = output_dir / 'text' / (keyid + '.txt')
                            # os.makedirs(os.path.split(textpath)[0], exist_ok=True)
                            # with open(textpath, "w") as f:
                            #     f.write(text)
