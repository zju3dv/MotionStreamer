...
from mld.models.losses.gpt import GPTLosses
...
from mld.data.utils import collate_tensors
...
class GPT(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        self.pkeep = cfg.model.pkeep
        self.nb_code = self.cfg.model.motion_vae.params.nb_code
        ...
        if self.stage == "gpt":
            if self.vae_type in ["mld", "vposert", "actor", "humanvq", "hvq", "hvq_body_hand"]:
                ...
                for p in self.motion_decoder.parameters():
                    p.requires_grad = False
        self.trans_encoder = instantiate_from_config(cfg.model.trans_encoder)
        ...
        # import pdb; pdb.set_trace()
        if cfg.TRAIN.use_tmr_supervision:
            self._get_tmr_encoders(cfg)
        ...
        if cfg.LOSS.TYPE == 'gpt':
            self._losses = MetricCollection({
                split: GPTLosses(cfg=cfg)
        ...
        elif eval("self.cfg.TRAIN.DATASETS")[0].lower() in ['motionx', 'motionx_v26']:
    ...
    def _get_t2m_evaluator(self, cfg):
        ...
        # import pdb; pdb.set_trace()
        if cfg.DATASET.MOTION_TYPE in ['vector_263']:
        ...
        elif cfg.DATASET.MOTION_TYPE == 'ric_rot':
            if self.input_format in ['all', 'root_body_pos_vel_hand_pos_vel']:
                self.t2m_moveencoder = t2m_motionenc.MovementConvEncoder(
                    input_size=cfg.DATASET.NFEATS - 4,
                    hidden_size=cfg.model.t2m_motionencoder.dim_move_hidden,
                    output_size=cfg.model.t2m_motionencoder.dim_move_latent,
        ...
        elif dataname == 'motionx_v26':
            # import pdb; pdb.set_trace()
            t2m_checkpoint = torch.load(
                os.path.join(cfg.model.t2m_path, dataname, cfg.DATASET.VERSION, cfg.DATASET.MOTION_TYPE, minor_motin_type, 
                            f"text_mot_match_glove_6B_caption_bs_256_text_{cfg.DATASET.TEXT_TYPE}/model/finest.tar"),  map_location=torch.device('cpu'))
        else:
            t2m_checkpoint = torch.load(
    ...
    def _get_tmr_encoders(self, cfg):
        """
        ...
        self.t2m_TMR_textencoder= DistilbertActorAgnosticEncoder('distilbert-base-uncased', num_layers=4)
        self.t2m_TMR_motionencoder = ActorAgnosticEncoder(nfeats=cfg.DATASET.NFEATS, vae =True, num_layers=4)
        ...
        if dataname == 'motionx':
            t2m_checkpoint = torch.load(
                os.path.join(cfg.model.t2m_path, dataname, cfg.DATASET.VERSION, cfg.DATASET.MOTION_TYPE, minor_motin_type, "TMR_pretrain/epoch=999.ckpt"),  map_location=torch.device('cpu'))
        ...
        self.t2m_TMR_textencoder.load_state_dict(tmr_textencoder_dict, strict=True)
        ...
        self.t2m_TMR_motionencoder.load_state_dict(tmr_motionencoder_dict, strict=True)

        # import pdb; pdb.set_trace()
        # freeze params
        self.t2m_TMR_textencoder.freeze()
        self.t2m_TMR_motionencoder.freeze()
    ...
    def _get_tmr_t2m_evaluator(self, cfg):
        ...
        if dataname == 'motionx':
            ...
            print('Load TMR_pretrain/epoch=59.ckpt !')
            state_dict = t2m_checkpoint["state_dict"]
        else:
            t2m_checkpoint = torch.load(
                os.path.join(cfg.model.t2m_path, dataname,
    ...
    def train_vae_forward(self, batch):
        ...
        if self.condition in ["text", "text_all", 'text_hand', 'text_body', 'text_face', "text_seperate", "only_pose_concat", "only_pose_fusion"]:
            ...
            if self.input_format in ['root_position', 'root_position_vel', 'root_position_rot6d', 'all', 'root_body_pos_vel_hand_all', 'root_body_pos_vel_hand_pos_vel', 'root_body_pos_vel_hand_pos', 'root_position_vel_only_body', 'root_body_pos_vel_hand_pos_vel_hand_wrist']:
                ...
                # import pdb; pdb.set_trace()
            elif self.input_format == 'smplx_212' and self.cfg.TRAIN.use_joints:
                ...
                joints_ref = self.feats2joints(feats_ref, skel=self.skel, motion_type=self.input_format)
                joints_ref = joints_ref.view(feats_ref.shape[0], feats_ref.shape[1], self.njoints, 3)
            elif self.input_format == 'smplx_212' and (not self.cfg.TRAIN.use_joints):
                pass
    ...
    def decomp_tokens(self, input):
        ...
        for index in range(input.shape[-1]):
            if index % 3 == 0 or index % 3 == 1:
                bottom_token.append(input[:, index])
            else:
                top_token.append(input[:, index])
        # import pdb; pdb.set_trace()
        return torch.tensor(bottom_token).to(input.device).unsqueeze(0), torch.tensor(top_token).to(input.device).unsqueeze(0)



    def train_gpt_forward(self, batch):
        # clip_text, m_tokens, m_tokens_len = batch
        
        m_tokens = batch['motion_token'] # 32, 51
        clip_text = batch['text']
        m_tokens_len = batch['motion_token_length'] # 32
        motion_length = batch['length']


        # import pdb; pdb.set_trace()
        assert self.trans_encoder.block_size == m_tokens.shape[1]
        # ########################### for debug ###################
        # self.vae.eval()
        # with torch.no_grad():
        #     # gt_data = np.load('/comp_robot/lushunlin/datasets/humanml3d/vq_tokens/humanvq/000000.npy')
        #     gt_data = m_tokens[1][:m_tokens_len[1]].unsqueeze(0)
            
        #     gt_data_bottom_token_1 = gt_data[:, 0::3]
        #     gt_data_bottom_token_2 = gt_data[:, 1::3]
        #     gt_data_bottom_token = torch.stack((gt_data_bottom_token_1.squeeze(), gt_data_bottom_token_2.squeeze()), dim=1)
        #     gt_data_bottom_token = gt_data_bottom_token.view(-1).unsqueeze(0)
        #     gt_data_top_token = gt_data[:, 2::3]
        #     import pdb; pdb.set_trace()
        #     # gt_data_bottom_token, gt_data_top_token = self.decomp_tokens(gt_data)
        #     # import pdb; pdb.set_trace()
        #     debug_pose_gt = self.vae.forward_decoder(gt_data_top_token, gt_data_bottom_token) # input: 15 # output: [1, 60, 263]
        #     debug_pose_gt = self.renorm2ori(debug_pose_gt)
        #     np.save("/comp_robot/lushunlin/visualization/visualization/test_case/hvq_gpt_debug/test_gpt_1_out_gt_token.npy", debug_pose_gt.squeeze(0).detach().cpu().numpy())
        # import pdb; pdb.set_trace()
        # ###########################for debug end ################
        ...
        # import pdb; pdb.set_trace()
        bs = m_tokens.shape[0]
        target = m_tokens # 32, 51
        # import pdb; pdb.set_trace()
        self.text_encoder.eval()
        with torch.no_grad():
            feat_clip_text = self.text_encoder(clip_text).squeeze(1) # (32, 512)
        ...
        if self.pkeep == -1:
            ...
            mask = torch.bernoulli(self.pkeep * torch.ones(input_index.shape,
                                                            device=input_index.device))

        # 
        mask = mask.round().to(dtype=torch.int64)
        r_indices = torch.randint_like(input_index, self.nb_code)
        a_indices = mask*input_index+(1-mask)*r_indices # 30, 50
        # import pdb; pdb.set_trace()
        cls_pred = self.trans_encoder(a_indices, feat_clip_text)
        cls_pred = cls_pred.contiguous()
        
        m_ref_token = []
        m_rst_token = []

        gen_supervise_tensor_list = []
        for i in range(bs):
            m_ref_token.append(target[i][:m_tokens_len[i] + 1])
            m_rst_token.append(cls_pred[i][:m_tokens_len[i] + 1])

            # loss function     (26), (26, 513)
            if self.cfg.TRAIN.use_tmr_supervision:
                # Go into the decoder supervision
                pred_idx = F.gumbel_softmax(cls_pred[i][:m_tokens_len[i], :-1], hard=True)
                # import pdb; pdb.set_trace()
                with torch.no_grad():
                    if self.vae_type in ['hvq', 'hvq_body_hand']:
                        # import pdb; pdb.set_trace()
                        if not pred_idx.shape[0] % 3 == 0:
                            pred_idx = pred_idx[:-(pred_idx.shape[0]%3)]
                        

                        # import pdb; pdb.set_trace()
                        gt_data_bottom_token_1 = pred_idx[0::3] # 
                        gt_data_bottom_token_2 = pred_idx[1::3]

                        gt_data_bottom_token = torch.cat([gt_data_bottom_token_1, gt_data_bottom_token_2], dim=-1).view(-1, gt_data_bottom_token_1.shape[-1]).unsqueeze(0)


                        gt_data_top_token = pred_idx[2::3].unsqueeze(0)
                        # import pdb; pdb.set_trace()

                        pred_pose = self.vae.forward_decoder(gt_data_top_token, gt_data_bottom_token, onehot=True)

                    else:
                        pred_pose = self.vae.forward_decoder(pred_idx, onehot=True)


                gen_supervise_tensor_list.append(pred_pose)

        if self.cfg.TRAIN.use_tmr_supervision:
            # import pdb; pdb.set_trace()
            supervise_motion = collate_tensors([b.squeeze() for b in gen_supervise_tensor_list])
            with torch.no_grad():
                supervise_motion_embedding = self.t2m_TMR_motionencoder(supervise_motion, motion_length).loc
                supervise_text_embedding = self.t2m_TMR_textencoder(clip_text).loc
                text_embedding = self.filter_model.encode(clip_text)
                text_embedding = torch.tensor(text_embedding).to(self.filter_model.device)
        ...
        if self.cfg.TRAIN.use_tmr_supervision:
            rs_set = {
                "m_ref": m_ref_token,
                "m_rst": m_rst_token, 
                "supervise_motion_feat": supervise_motion_embedding, 
                "supervise_text_feat": supervise_text_embedding, 
                "emb_dist": emb_dist
            }     
        else:
            rs_set = {
    ...
    def t2m_eval(self, batch):
        ...
        m_tokens = batch['motion_token'].detach().clone()
        m_tokens_len = batch['motion_token_length']
        ...
        if self.trainer.datamodule.is_mm:
            texts = texts * self.cfg.TEST.MM_NUM_REPEATS # 30
            motions = motions.repeat_interleave(self.cfg.TEST.MM_NUM_REPEATS,
                                                dim=0)
            lengths = lengths * self.cfg.TEST.MM_NUM_REPEATS # 30
        ...
        elif self.stage in ['gpt']:
            with torch.no_grad():
                feat_clip_text = self.text_encoder(texts).squeeze(1) # (32, 768)
                pred_pose_eval = torch.zeros((bs, seq, motions.shape[-1])).to(feat_clip_text.device) # 1, 192, 313
                pred_len = torch.ones(bs).long()
                # import pdb; pdb.set_trace()
                # motion_ref_token_list = []
                # motion_rst_token_list = []
                for k in range(bs):
                    try:
                        index_motion = self.trans_encoder.sample(feat_clip_text[k:k+1], False) # [1,23]
                    except:
                        if self.vae_type in ['hvq', 'hvq_body_hand']:
                            # import pdb; pdb.set_trace()
                            index_motion = torch.ones(1,3).to(feat_clip_text.device).long()
                        else:
                            index_motion = torch.ones(1,1).to(feat_clip_text.device).long()
                    # if index_motion[:, :-(index_motion.shape[1]%3)].shape[1] == 0:
                    #     import pdb; pdb.set_trace()
                    # import pdb; pdb.set_trace()
                    # motion_ref_token_list.append(m_tokens[k][:m_tokens_len[k] + 1])
                    # motion_rst_token_list.append(index_motion[k][:m_tokens_len[k] + 1])
                    ...
                    if self.vae_type in ['hvq', 'hvq_body_hand']:
                        # import pdb; pdb.set_trace()
                        if index_motion.shape[1] < 3:
                            index_motion = torch.ones(1,3).to(feat_clip_text.device).long()

                        if not index_motion.shape[1] % 3 == 0:
                            index_motion_ori = index_motion.clone()
                            index_motion = index_motion[:, :-(index_motion.shape[1]%3)]
                        

                        # import pdb; pdb.set_trace()
                        gt_data_bottom_token_1 = index_motion[:, 0::3]
                        gt_data_bottom_token_2 = index_motion[:, 1::3]
                        assert gt_data_bottom_token_1.shape[0] == 1
                        assert gt_data_bottom_token_2.shape[0] == 1
                        gt_data_bottom_token = torch.stack((gt_data_bottom_token_1.view(gt_data_bottom_token_1.shape[1]), gt_data_bottom_token_2.view(gt_data_bottom_token_2.shape[1])), dim=1)
                        gt_data_bottom_token = gt_data_bottom_token.view(-1).unsqueeze(0)

                        gt_data_top_token = index_motion[:, 2::3]
                        # import pdb; pdb.set_trace()
                        try:
                            pred_pose = self.vae.forward_decoder(gt_data_top_token, gt_data_bottom_token)
                        except:
                            import pdb; pdb.set_trace()

                    else:
                        pred_pose = self.vae.forward_decoder(index_motion)
        ...
        #         feats_rst = self.vae.decode(z, lengths)
        #     elif self.vae_type in ["humanvq", "spatial_MLP_vqvae", "spatial_transformer_vqvae"]:
        #         feats_rst = self.vae.forward_decoder(index_motion)
        #         import pdb; pdb.set_trace()
        ...
        # import pdb; pdb.set_trace()
        # joints recover
        joints_rst = self.feats2joints(pred_pose_eval, self.input_format)
        joints_ref = self.feats2joints(motions, self.input_format)

        import pdb; pdb.set_trace()
        # renorm for t2m evaluators
        pred_pose_eval = self.datamodule.renorm4t2m(pred_pose_eval)
        ...
        pred_pose_eval = pred_pose_eval[align_idx]
        ...
        rs_set = {
            "m_ref": motions,
            "m_rst": pred_pose_eval,
            "lat_t": text_emb,
            "lat_m": motion_emb,
            "lat_rm": recons_emb,
            "joints_ref": joints_ref,
            "joints_rst": joints_rst,
            # "motion_ref_token": motion_ref_token_list, 
            # "motion_rst_token": motion_rst_token_list,
            # "motion_token_length": m_tokens_len
        }
        return rs_set
    ...
    def t2m_eval_save_motion_token(self, batch):
        ...
        if self.stage in ['gpt']:
            with torch.no_grad():
                feat_clip_text = self.text_encoder(texts).squeeze(1) # (32, 768)
                pred_pose_eval = torch.zeros((bs, seq, motions.shape[-1])).to(feat_clip_text.device)
                pred_len = torch.ones(bs).long()
                # import pdb; pdb.set_trace()
                # motion_ref_token_list = []
                # motion_rst_token_list = []
        ...
        joints_rst = self.feats2joints(pred_pose_eval, skel=self.skel, motion_type=self.input_format)
        # import pdb; pdb.set_trace()

        joints_rst = joints_rst.view(pred_pose_eval.shape[0], pred_pose_eval.shape[1], self.njoints, 3)
    ...
    def tmr_t2m_eval(self, batch):
        ...
        with torch.no_grad():
            ...
            for k in range(bs):
                try:
                    index_motion = self.trans_encoder.sample(feat_clip_text[k:k+1], True) # [1,23]
                except:
                    if self.vae_type in ['hvq', 'hvq_body_hand']:
                        # import pdb; pdb.set_trace()
                        index_motion = torch.ones(1,3).to(feat_clip_text.device).long()
        ...
        # #########################saving output###################
        # renorm for t2m evaluators
        feats_rst_before_renorm4t2m = pred_pose_eval.clone()
        ...
        text_emb_tmr = self.t2m_TMR_textencoder_eval(texts).loc[align_idx]
        # import pdb; pdb.set_trace()
    ...
    def allsplit_step(self, split: str, batch, batch_idx):
        ...
        if split in ["train", "val"]:
            ...
            if self.stage in ['gpt']:
                # if self.cfg.model.gpt_type in ['body_hand_gpt']:
                #     rs_set = self.train_body_hand_gpt_forward(batch)
                # else:
                rs_set = self.train_gpt_forward(batch)
        ...
        if split in ["val", "test"]:
            ...
            if self.condition in ['text', 'text_uncond', 'text_all', 'text_face', 'text_body', 'text_hand', 'text_face_body', 'text_seperate', 'only_pose_concat', 'only_pose_fusion']:
                ...
                if self.input_format in ['vector_263', 'root_body_pos_vel_hand_pos_vel']:
                    if self.condition == 'text':
                        ...
                        raise NotImplementedError
                elif self.input_format == 'smplx_212':
                    if self.condition == 'text':
            ...
            for metric in metrics_dicts:
                if metric == "TemosMetric":
                    ...
                    getattr(self, metric).update(
                        rs_set["motion_rst_token"], 
                        rs_set["motion_ref_token"], 
                        rs_set["motion_token_length"]
                        )
                else:
