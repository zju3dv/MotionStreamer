...
logger.info(f'fid: {fid}, avg. {sum(fid)/repeat_time}')
logger.info(f'mpjpe: {mpjpe} (mm), avg. {sum(mpjpe)/repeat_time} (mm)')

fid = np.array(fid)

mpjpe_cpu = [item.cpu().numpy() for item in mpjpe]
mpjpe = np.array(mpjpe_cpu)

msg_final = f"FID. {np.mean(fid):.4f}, conf. {np.std(fid)*1.96/np.sqrt(repeat_time):.3f}, MPJPE. {np.mean(mpjpe):.5f} (mm), conf. {np.std(mpjpe)*1.96/np.sqrt(repeat_time):.3f}"
